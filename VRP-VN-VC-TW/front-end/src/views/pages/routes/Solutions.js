...
import { getCustomersOfJourney, secondsToHHMMSS } from '../../../service/utility'
import routeService from '../../../service/RouteService'
import CIcon from '@coreui/icons-react'
import SaveSolutionRouting from './solution-routing/SaveSolutionRouting'
                    ...
                    {this.state.isShowConfirm === true &&
                        ...
                        >
                            <CModalHeader closeButton>
                                <CModalTitle>Confirm Change Order</CModalTitle>
                            </CModalHeader>
                            <CModalBody>
                                <h6>Xác nhận chuyển đơn hàng <b>{this.state.changedOrder.code}</b> từ hành trình của <b>{this.state.fromJourney.vehicle.name}</b> tới hành trình của <b>{this.state.toJourney.vehicle.name}</b> ?</h6>
                            </CModalBody>
                            <CModalFooter>
                                {this.state.isWaiting === true &&
                                    <CSpinner size='sm' color='green'></CSpinner>
                                }
                                <CButton color="success" onClick={() => this.handleChangeSubmit()}>Confirm</CButton>{' '}
                                <CButton color="secondary" onClick={() => this.setShowConfirm(false)}>Cancel</CButton>
                            </CModalFooter>
                        </CModal>
                    }
                        ...
                        <CCard>
                            <CCardHeader>
                                <CRow>
                                    <CCol sm='4'>
                                        <CFormGroup row>
                                            {this.state.solution &&
                                                <CCol md='5'>
                                                    <CLabel htmlFor="solution">{"Timeline of Solution: " + this.state.solution.id}</CLabel>
                                                </CCol>
                                            }
                                            {this.state.nodesTimeline &&
                                                <CCol sm='5'>
                                                    <CSelect custom size="sm" name="solution" id="solution" value={this.state.solution.id} onChange={this.handleChangeSolution} >
                                                        {this.state.solutions.map((item, index) => <option key={index} value={item.id}>{"Solution " + (item.id)}</option>)}
                                                    </CSelect>
                                                </CCol>
                                            }
                                        </CFormGroup>
                                    ...
                                    <CCol sm='5'>
                                        <CFormGroup row>
                                            {this.state.solution &&
                                                <CCol md='3'>
                                                    <CLabel htmlFor="journey">{"Journey: "}</CLabel>
                                                </CCol>
                                            }
                                            {this.state.solutions &&
                                                <CCol sm='6'>
                                                    <CSelect custom size="sm" name="journey" id="journey" value={this.state.solution.focusJourneyId} onChange={this.handleChangeJourney} >
                                                        <option key={0} value={0}>{"All journey"}</option>
                                ...
                                {this.state.solution.focusJourneyId === 0 &&
                                    <CRow>
                                        <CCol sm='2'>{"Tổng chi phí: " + Math.round((this.state.solution.totalCost) * 10) / 10 + " (VND)"}</CCol>
                                        <CCol sm='2'>{"Doanh thu: " + this.state.solution.totalAmount + " (VND)"}</CCol>
                                        <CCol sm='2'>{"Lợi nhuận: " + Math.round((this.state.solution.revenue) * 10) / 10 + " (VND)"}</CCol>
                                        <CCol sm='2'>{"Hiệu suất: " + Math.round((this.state.solution.efficiency) * 100) / 100 + "%"}</CCol>
                                        <CCol sm='2'>{"Số xe: " + this.state.solution.numberVehicle + " xe"}</CCol>
                                    </CRow>
                                }
                                {this.state.solution.focusJourneyId === 0 &&
                                    <CRow>
                                        <CCol sm='3'>{"Số đơn trung bình: " + Math.round(((this.state.solution.totalNumberOrders) / this.state.solution.numberVehicle)) + " (đơn/xe)"}</CCol>
                                        <CCol sm='3'>{"Thời gian đi trung bình: " + (Math.round(((this.state.solution.totalTravelTime / 60) / this.state.solution.numberVehicle) * 10) / 10) + " (phút/xe)"}</CCol>
                                        <CCol sm='3'>{"Quãng đường trung bình: " + Math.round(((this.state.solution.totalDistance / 1000) / this.state.solution.numberVehicle) * 10) / 10 + " (km)"}</CCol>
                                    </CRow>
                                }
                                {this.state.solution.focusJourneyId !== 0 &&
                                    <CRow>
                                        <CCol sm='2'>{"Tổng chi phí: " + Math.round((this.state.focusJourney.totalCost) * 10) / 10 + " (VND)"}</CCol>
                                        <CCol sm='2'>{"Phí phạt: " + Math.round((this.state.focusJourney.totalPenaltyCost) * 10) / 10 + " (VND)"}</CCol>
                                        <CCol sm='2'>{"Doanh thu: " + this.state.focusJourney.totalAmount + " (VND)"}</CCol>
                                        <CCol sm='2'>{"Lợi nhuận: " + Math.round((this.state.focusJourney.revenue) * 10) / 10 + " (VND)"}</CCol>
                                        <CCol sm='2'>{"Hiệu suất: " + Math.round((this.state.focusJourney.revenue * 100 / this.state.focusJourney.totalAmount) * 100) / 100 + "%"}</CCol>
                                    </CRow>
                                }
                                {this.state.solution.focusJourneyId !== 0 &&
                                    <CRow>
                                        <CCol sm='2'>{"Số đơn: " + getCustomersOfJourney(this.state.focusJourney).length + " đơn"}</CCol>
                                        <CCol sm='2'>{"Số chuyến: " + this.state.focusJourney.routes.length + " chuyến"}</CCol>
                                    </CRow>
                                }
                            </CCardHeader>
